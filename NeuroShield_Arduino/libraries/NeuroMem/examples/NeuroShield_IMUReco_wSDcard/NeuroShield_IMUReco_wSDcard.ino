...
void setup()
{
  ...
  if (hNN.begin(HW_NEUROSHIELD) == 0) 
  {
    ...
    int error=hNN.loadKnowledge_SDcard("Default.knf");
    if (error==0)
    {
      Serial.print("Knowledge loaded from SD card");
      ncount= hNN.NCOUNT();
      if (ncount!=0) {Serial.print("\nCommitted neurons = "); Serial.print(ncount);}
    }
    else
    {
...
void processSerialEvent() 
{
    ...
    else
    {
      ...
      {
        Serial.println("\nSaving...");
        int error=hNN.saveKnowledge_SDcard("UserIMU.knf");
        if (error==0)Serial.print("\nKnowledge saved");
        else {Serial.print("\nProblem saving the knowledge, error ");Serial.println(error);}
      }
      else if (inputString=="rkn\n")
      {
        Serial.println("\nLoading...");
        int error=hNN.loadKnowledge_SDcard("UserIMU.knf");
        if (error==0)Serial.print("\nKnowledge loaded");
        else  {Serial.print("\nProblem loading the knowledge, error ");Serial.println(error);}
      }
